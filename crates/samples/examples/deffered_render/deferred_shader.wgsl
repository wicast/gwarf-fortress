...
fn fs_main(in: VertexOutput) -> FragOut {
    var out: FragOut;
    let pos = textureSample(pos_gb, s, in.uv).xyz;
    let normal = textureSample(normal_gb, s, in.uv).xyz;
    ...
    let view_direction = normalize(camera.view_pos.xyz - pos);
    ...
    let out_color = albedo.xyz * (ambient + diffuse + specular);
